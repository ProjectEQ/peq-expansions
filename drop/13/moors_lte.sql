# INSERT INTO loottable_entries(probability, loottable_id, lootdrop_id, drop_limit, mindrop) VALUES (100, :todo_lt:, :todo_ld:, 1, 1);

INSERT INTO loottable_entries(probability, loottable_id, lootdrop_id, drop_limit, mindrop) VALUES 

(1, :moors_a_mucktail_foreman_lt:, :cloth_small_ld:),
(5, :moors_a_mucktail_foreman_lt:, :rusty_ld:),

(1, :moors_a_mucktail_miner_lt:, :cloth_small_ld:),
(5, :moors_a_mucktail_miner_lt:, :rusty_ld:),

(1, :moors_a_mucktail_rock_breaker_lt:, :cloth_small_ld:),
(5, :moors_a_mucktail_rock_breaker_lt:, :rusty_ld:),

(1, :moors_chaplain_bloodmoon_lt:, :cloth_small_ld:),
(5, :moors_chaplain_bloodmoon_lt:, :rusty_ld:),

(1, :moors_defan_the_watch_captain_lt:, :cloth_small_ld:),
(5, :moors_defan_the_watch_captain_lt:, :rusty_ld:),

(1, :moors_ezzerak_the_engineer_lt:, :cloth_small_ld:),
(5, :moors_ezzerak_the_engineer_lt:, :rusty_ld:),

(1, :moors_pit_boss_scar_lt:, :cloth_small_ld:),
(5, :moors_pit_boss_scar_lt:, :rusty_ld:),
(1, :moors_a_mucktail_miner_lt:, :rawhide_medium_ld:),
(1, :moors_ezzerak_the_engineer_lt:, :rawhide_medium_ld:),
(1, :moors_pit_boss_scar_lt:, :rawhide_medium_ld:),
(1, :moors_a_mucktail_foreman_lt:, :rawhide_medium_ld:),
(1, :moors_chaplain_bloodmoon_lt:, :rawhide_medium_ld:),
(1, :moors_a_mucktail_rock_breaker_lt:, :rawhide_medium_ld:),
