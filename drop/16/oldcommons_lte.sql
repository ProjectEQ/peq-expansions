# INSERT INTO loottable_entries(probability, loottable_id, lootdrop_id, drop_limit, mindrop) VALUES (100, :todo_lt:, :todo_ld:, 1, 1);

INSERT INTO loottable_entries(probability, loottable_id, lootdrop_id, drop_limit, mindrop) VALUES 
(20, :oldcommons_a_young_kodiak_lt:, :bear_meat_ld:)
(4, :oldcommons_a_young_kodiak_lt:, :bear_parts_ld:)
(25, :oldcommons_a_young_kodiak_lt:, :bear_skin_ld:)

(18, :oldcommons_a_black_bear_lt:, :bear_meat_ld:)
(2, :oldcommons_a_black_bear_lt:, :bear_parts_ld:)
(25, :oldcommons_a_black_bear_lt:, :bear_skin_ld:)

(20, :oldcommons_an_enraged_kodiak_lt:, :bear_meat_ld:)
(4, :oldcommons_an_enraged_kodiak_lt:, :bear_parts_ld:)
(25, :oldcommons_an_enraged_kodiak_lt:, :bear_skin_ld:)

(1, :oldcommons_a_decaying_skeleton_lt:, :cloth_small_ld:),
(5, :oldcommons_a_decaying_skeleton_lt:, :rusty_ld:),

(1, :oldcommons_a_dervish_cutthroat_lt:, :cloth_small_ld:),
(5, :oldcommons_a_dervish_cutthroat_lt:, :rusty_ld:),

(1, :oldcommons_a_skeleton_lt:, :cloth_small_ld:),
(5, :oldcommons_a_skeleton_lt:, :rusty_ld:),

(1, :oldcommons_an_orc_centurion_lt:, :cloth_small_ld:),
(5, :oldcommons_an_orc_centurion_lt:, :rusty_ld:),

(1, :oldcommons_an_orc_legionnaire_lt:, :cloth_small_ld:),
(5, :oldcommons_an_orc_legionnaire_lt:, :rusty_ld:),

(1, :oldcommons_high_chief_fosloas_lt:, :cloth_small_ld:),
(5, :oldcommons_high_chief_fosloas_lt:, :rusty_ld:),

(1, :oldcommons_orc_centurion_lt:, :cloth_small_ld:),
(5, :oldcommons_orc_centurion_lt:, :rusty_ld:),
(1, :oldcommons_an_obliteration_acolyte_lt:, :fine_steel_ld:),
(1, :oldcommons_an_obliteration_conjurer_lt:, :fine_steel_ld:),
(1, :oldcommons_an_obliteration_warlord_lt:, :fine_steel_ld:),
(1, :oldcommons_an_army_of_light_blademaster_lt:, :fine_steel_ld:),
(1, :oldcommons_rinna_lightshadow_lt:, :fine_steel_ld:),
(1, :oldcommons_merrin_beanslinger_lt:, :fine_steel_ld:),
(1, :oldcommons_an_obliteration_weaponmaster_lt:, :fine_steel_ld:),
(1, :oldcommons_an_obliteration_oracle_lt:, :fine_steel_ld:),
(1, :oldcommons_an_obliteration_nightblade_lt:, :fine_steel_ld:),
(1, :oldcommons_an_obliteration_soldier_lt:, :fine_steel_ld:),
(1, :oldcommons_an_obliteration_warlock_lt:, :fine_steel_ld:),
(1, :oldcommons_tallyd_gorenault_lt:, :fine_steel_ld:),
(1, :oldcommons_an_obliteration_knight_lt:, :fine_steel_ld:),
(1, :oldcommons_an_obliteration_assassin_lt:, :fine_steel_ld:),
(1, :oldcommons_a_hill_giant_lt:, :fine_steel_ld:),
(1, :oldcommons_an_army_of_light_sorceror_lt:, :fine_steel_ld:),
(1, :oldcommons_an_obliteration_evoker_lt:, :fine_steel_ld:),
(1, :oldcommons_shadow_treebright_lt:, :fine_steel_ld:),
(1, :oldcommons_an_obliteration_summoner_lt:, :fine_steel_ld:),
(1, :oldcommons_an_orc_legionnaire_lt:, :rawhide_medium_ld:),
(1, :oldcommons_shadow_treebright_lt:, :rawhide_medium_ld:),
(1, :oldcommons_a_ghoul_lt:, :rawhide_medium_ld:),
